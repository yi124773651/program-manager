...
[![Version](https://img.shields.io/badge/Version-1.1.6-blue?style=flat-square)](https://github.com/yi124773651/program-manager/releases)
...
### 效率工具
- 🎬 **场景工作流** - 一键执行多个操作，支持 8 种动作类型、失败策略和执行日志
- 📋 **剪贴板历史** - 自动记录剪贴板内容，支持置顶
- 🔍 **快捷搜索** - Spotlight 风格的全局搜索，支持应用、剪贴板、场景、便签、待办和网页搜索
- 📝 **快捷便签** - 随时记录灵感和待办事项
- ✅ **待办日程表** - 独立待办窗口，支持日期、完成状态和快捷搜索联动
- 🧮 **计算器增强** - 在搜索框中直接计算表达式
- ⌨️ **可配置快捷键** - 支持全局唤起、快捷搜索、便签和待办快捷键设置、冲突提示与恢复默认
- 🎨 **个性化** - 自定义主题色、背景图片（本地/随机图床）、窗口透明度

### 维护工具
- 🧹 **清理无效项** - 一键检测失效程序，批量删除前确认具体条目
- 🔄 **更新检测** - 智能检测程序更新，支持版本号和文件元数据混合检测
- 📦 **本地导入导出** - 导出完整本地数据包，支持导入预览、选择性覆盖和导入前备份
- 💾 **统一本地存储** - 场景、便签、待办、剪贴板和动作配置使用 JSON 文件持久化，旧数据自动迁移
...
构建产物通常位于：`src-tauri/target/release/bundle/nsis/`。如果设置了 `CARGO_TARGET_DIR`，产物会出现在对应 target 目录下。
...
- `CI`：分支 push 和指向 `main` 的 pull request 触发，执行版本一致性检查、前端测试、前端构建、Rust 格式检查、Rust 测试和 `cargo check`，不打 Tauri 安装包。
- `Release`：仅推送 `v*` tag 时触发，要求 tag 版本与 `package.json` / `src-tauri/tauri.conf.json` 一致，然后构建并发布 Tauri 安装包。
- 发布流程不再自动递增版本号；版本号调整、变更记录和 tag 创建需要在发布前手工完成。
...
| 快捷便签 | Alt+N | 快速记录笔记 |
| 待办日程表 | Alt+T | 打开或隐藏独立待办窗口 |
| 计算器 | - | 搜索框中直接计算 |

所有全局快捷键都可以在设置中修改。保存失败时会保留旧快捷键，并展示冲突或注册失败原因。

### 本地数据导入导出

在设置页的数据导入导出区域可以：

1. 导出完整本地数据包，包含 `manifest.json`、配置文件、场景、便签、待办、剪贴板、动作配置和 `icons/`。
2. 选择数据包中的 `manifest.json` 进行导入预览。
3. 按数据类型选择性覆盖导入。
4. 导入前自动备份当前数据，导入失败时尝试恢复备份。
...
<br/>
主要文件包括 <code>config.json</code>、<code>scenes.json</code>、<code>notes.json</code>、<code>todos.json</code>、<code>clipboard.json</code>、<code>actions.json</code>、<code>migrations.json</code>、<code>icons/</code> 和 <code>backups/</code>。
</details>
...
<br/>
场景数据现在存储在 <code>%APPDATA%/program-manager/scenes.json</code>。旧版本 localStorage 数据会在启动后迁移到本地 JSON 文件，迁移前会自动备份。
</details>
